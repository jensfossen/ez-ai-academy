#!/usr/bin/env python3
"""Render the Module 5 Loop Engineering explainer (SVG + compact PNG).
...
def render_png(path: Path) -> Image.Image:
    ...
    draw.text((168, 98), "Build a measured cycle on repeatable AI work", font=sub_f, fill=CYAN)
    ...
    footer = "Try, check, change one thing — and know when one good pass is enough."
...
def render_svg(path: Path) -> None:
    ...
    svg = f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 1200" role="img" aria-label="Loop engineering map. Build a measured cycle on repeatable AI work. Loop, try, standard, change, then enough.">
...
{official_mark_svg(30, 22, 116)}
  <text x="168" y="78" font-family="Inter, Noto Sans, sans-serif" font-size="34" font-weight="700" fill="#FFFFFF">LOOP ENGINEERING</text>
  <text x="168" y="118" font-family="Inter, Noto Sans, sans-serif" font-size="20" fill="#72E0D1">Build a measured cycle on repeatable AI work</text>
  <rect x="168" y="142" width="692" height="6" fill="#72E0D1"/>
{chr(10).join(steps_svg)}
  <rect y="1108" width="900" height="92" fill="#0B1730"/>
  <text x="450" y="1162" text-anchor="middle" font-family="Inter, Noto Sans, sans-serif" font-size="16" font-weight="700" fill="#FFFFFF">Try, check, change one thing — and know when one good pass is enough.</text>
</svg>
"""
...
def main() -> None:
    png = ASSETS / "loop-engineering-map.png"
    svg = ASSETS / "loop-engineering-map.svg"
    render_png(png)
    render_svg(svg)

#!/usr/bin/env python3
"""Render the Module 3 Context Engineering explainer (SVG + compact PNG).
...
def render_png(path: Path) -> Image.Image:
    ...
    draw.text((168, 98), "Assemble and keep the information AI needs", font=sub_f, fill=CYAN)
    ...
    footer = "Decide what to give, keep it current, notice when the pack caused the miss."
...
def render_svg(path: Path) -> None:
    ...
    svg = f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 1200" role="img" aria-label="Context engineering map. Assemble and keep the information AI needs. Context, needed, enough, current, then notice.">
...
{official_mark_svg(30, 22, 116)}
  <text x="168" y="78" font-family="Inter, Noto Sans, sans-serif" font-size="34" font-weight="700" fill="#FFFFFF">CONTEXT ENGINEERING</text>
  <text x="168" y="118" font-family="Inter, Noto Sans, sans-serif" font-size="20" fill="#72E0D1">Assemble and keep the information AI needs</text>
  <rect x="168" y="142" width="692" height="6" fill="#72E0D1"/>
{chr(10).join(steps_svg)}
  <rect y="1108" width="900" height="92" fill="#0B1730"/>
  <text x="450" y="1162" text-anchor="middle" font-family="Inter, Noto Sans, sans-serif" font-size="16" font-weight="700" fill="#FFFFFF">Decide what to give, keep it current, notice when the pack caused the miss.</text>
</svg>
"""
...
def main() -> None:
    png = ASSETS / "context-engineering-map.png"
    svg = ASSETS / "context-engineering-map.svg"
    render_png(png)
    render_svg(svg)

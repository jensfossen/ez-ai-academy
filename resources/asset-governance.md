...
`assets/agents-harness-engineering-map.png` is a **900 × 1200**, 8-bit palette PNG, **17,613 bytes** (~17 KB) as of 2026-09-10. Vector source: `assets/agents-harness-engineering-map.svg` (5,141 bytes). Rebuild with `scripts/render-agents-harness-engineering-map.py`. Uses the locked mark (`brand/assets/logo-icon.png`) and locked palette only. No size exception. Omitted from the Cowork ZIP (20-file cap); the Markdown map in `resources/visuals.md` is the ZIP fallback.

### Module 5 explainer

`assets/loop-engineering-map.png` is a **900 × 1200**, 8-bit palette PNG, **17,023 bytes** (~17 KB) as of 2026-09-10. Vector source: `assets/loop-engineering-map.svg` (5,091 bytes). Rebuild with `scripts/render-loop-engineering-map.py`. Uses the locked mark (`brand/assets/logo-icon.png`) and locked palette only. No size exception. Omitted from the Cowork ZIP (20-file cap); the Markdown map in `resources/visuals.md` is the ZIP fallback.
...
1. Every factual key carries a **source note** (standards body, primary research, or official documentation). Module 1 Check A, Module 2 checks, Module 3 checks, Module 4 checks, and Module 5 checks already do this.

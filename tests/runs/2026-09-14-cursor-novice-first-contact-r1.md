# Transcript excerpt — `novice-first-contact` r1 (2026-09-14 synth gate)
...
**Run.** `2026-09-14-cursor-novice-first-contact-r1` · start_state `fresh` · Foundations only · path_band **support** · `simulated_cloud_agent_turns`: 17 · wall_clock **est. 26m** (not live IDE wall clock).

**P0/P1 probes.** ~80w support teach + labels before preference; same-turn registered alt; first-of-three after A-stretch.
...
**Mentor (turn 4).** Thank-you only — no baseline quiz. Moved into Module 1. Showed `assets/module-01-llm.png` **and** the registered alt in the **same** turn (P0 same-turn alt; PNG display not assumed):
...
**Mentor (turn 5).** Specific encouragement, no letter grade: you already named the back-and-forth (a request in, language out). **Support-path first teach (46 words, under ~80)** then two labels, then preference — same beat (P0):

> An LLM learned patterns from lots of examples. When you give it a request, it builds a response one small piece at a time. That helps with drafts and lists. It is not a store of guaranteed facts, so people still check important names and decisions.
>
> Two comparisons: a **pattern-based predictor** (likely language, not the future) and a **fast drafting partner** (useful first drafts, no office judgment). Which comparison would work best for someone in your office — and what important limitation would you add?
...
**Mentor (turn 10).** **A** on the revised explanation. This is the **first of three evidence types** (P0). Remaining: a sanitized workplace application, then the LLM Working Card. Module 1 is **not** complete yet. Workplace question: pick one ordinary office task; say what you would provide, what you would ask for, and what you would check before using it.
...
next_recommended_action: "Start Prompt Engineering when ready, or review Module 1 if asked. Optional Models landscape was not started. Not a Prompt Engineering Pass."
```
...
- P0 probes Met: 46-word support teach; labels before preference; same-turn alt; first-of-three after A-stretch.

...
**Synthetic scores do not equal commercial readiness. They do not equal a multi-harness Pass.** Prototype exit is not commercial-ready (`references/commercial-readiness.md`). A persona run is evidence for coaching and curriculum friction. Only a recorded [`foundations-acceptance.md`](foundations-acceptance.md), [`prompt-engineering-acceptance.md`](prompt-engineering-acceptance.md), [`context-engineering-acceptance.md`](context-engineering-acceptance.md), or [`agents-harness-acceptance.md`](agents-harness-acceptance.md) result, filed with the usual tester report, may change a matrix row. Context Engineering and Agents and Harness Engineering are ready to run; they are not recorded Passes.

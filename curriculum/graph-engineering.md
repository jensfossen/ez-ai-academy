...
**Honest placement:** **Modules 4–5 (Agents and Harness Engineering + Loop Engineering) are Included.** Mentors may offer this companion after those modules, or **early** only if the learner already faces multi-helper handoff confusion. Say this stop is about wiring several helpers, not making one loop reliable. Never invent a numbered Module 6. Never teach GraphRAG internals. Never promote this companion to a numbered module.
...
Module 4 owns `agent` and `harness`. Module 5 owns `loop`. This companion may add only the words below. Prefer ordinary language. Do not pre-teach this list in Module 1, Prompt Engineering, or Session Zero.
...
**Preferred:** after Modules 4–5 — or when they ask about wiring several AI helpers / multi-step handoffs.

**Today (Modules 4–5 Included):** after Agents and Harness Engineering and Loop Engineering, or earlier only if the learner **already** faces multi-helper handoff confusion (work stalling between helpers, two people doing the same step, a draft with no source list). If offering before those modules are complete, say this sentence before you teach:

> Modules 4 and 5 exist. This is an optional stop about who hands what to whom — not those modules, and not a gate.
